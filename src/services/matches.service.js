...
const queriesForHomeList = async (options) => {
    try {
        ...
        const currentDate = new Date();
        const startOfDay = new Date(currentDate.setHours(0, 0, 0, 0));
        const fourDaysLater = new Date(startOfDay.getTime() + 4 * 24 * 60 * 60 * 1000);
        const threeDayBefore = new Date(startOfDay.getTime() - 3 * 24 * 60 * 60 * 1000);
                    ...
                    pipeline: [
                        ...
                        },
                        {
                            $unionWith: {
                                coll: "matches",
                                pipeline: [
                                    // Stage 3: Finished matches from the last 3 days
                                    {
                                        $match: {
                                            match_status: "Finished",
                                            $expr: {
                                                $and: [
                                                    { $gte: ["$date_time", threeDayBefore] },
                                                    { $lt: ["$date_time", startOfDay] }
                                                ]
                                            }
                                        }
                                    },
